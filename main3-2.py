...
def make_phone(content):
    outer = AnchorLayout(anchor_x="center", anchor_y="center", padding=12)
    ...
    status = BoxLayout(orientation="horizontal", size_hint_y=None, height=28)
    status.add_widget(Label(text="9:41", color=TEXT, bold=True, font_size=13))
    status.add_widget(Label(text="Smart Planner", color=MUTED, font_size=12))
    status.add_widget(Label(text="● ● ●", color=TEXT, font_size=11))
    phone.add_widget(status)
...
def message_popup(title, message, color=PRIMARY):
    ...
    content.add_widget(Label(text=message, font_size=16, color=TEXT))

    close_button = RoundedButton("OK", color, size_hint_y=None, height=46)
    content.add_widget(close_button)

    popup = Popup(title=title, content=content, size_hint=(0.78, 0.36), auto_dismiss=False)
...
class LoginScreen(Screen):
    def __init__(self, **kwargs):
        ...
        card = RoundedBox(orientation="vertical", bg_color=CARD, radius=22, size_hint_y=None, height=360)

        card.add_widget(Label(text="Welcome Back", font_size=22, bold=True, color=TEXT, size_hint_y=None, height=48))
        ...
        self.message = Label(text="", color=DANGER, font_size=13, size_hint_y=None, height=34)
        card.add_widget(self.message)

        login_button = RoundedButton("LOGIN", PRIMARY, size_hint_y=None, height=48)
        login_button.bind(on_press=self.login_user)
        card.add_widget(login_button)

        signup_button = RoundedButton("CREATE ACCOUNT", SUCCESS, size_hint_y=None, height=48)
        signup_button.bind(on_press=self.open_signup)
        card.add_widget(signup_button)

        content.add_widget(card)
        content.add_widget(Label(text="Organise deadlines, modules, and priorities.", color=MUTED, font_size=13))
...
class SignupScreen(Screen):
    def __init__(self, **kwargs):
        ...
        content.add_widget(Label(text="Create Account", font_size=26, bold=True, color=PRIMARY, size_hint_y=None, height=70))

        card = RoundedBox(orientation="vertical", bg_color=CARD, radius=22)
        ...
        self.message = Label(text="", color=DANGER, font_size=13, size_hint_y=None, height=35)
        card.add_widget(self.message)

        signup_button = RoundedButton("SIGN UP", SUCCESS, size_hint_y=None, height=48)
        signup_button.bind(on_press=self.signup_user)
        card.add_widget(signup_button)

        back_button = RoundedButton("BACK TO LOGIN", SECONDARY, size_hint_y=None, height=48)
    ...
    def signup_user(self, instance):
        ...
        users[username] = {"full_name": full_name, "password": hash_password(password)}
        ...
        message_popup("Success", "Account created successfully.", SUCCESS)
        self.manager.current = "login"
...
class DashboardScreen(ProtectedScreen):
    def __init__(self, **kwargs):
        ...
        content.add_widget(Label(text="Dashboard", font_size=28, bold=True, color=PRIMARY, size_hint_y=None, height=60))

        summary = RoundedBox(orientation="vertical", bg_color=CARD, radius=22, size_hint_y=None, height=145)
        self.welcome = Label(text="", font_size=18, bold=True, color=TEXT)
        self.task_summary = Label(text="", color=MUTED, font_size=14)
        summary.add_widget(self.welcome)
        summary.add_widget(self.task_summary)
        content.add_widget(summary)

        task_button = RoundedButton("TASK MANAGEMENT", PRIMARY, size_hint_y=None, height=52)
        task_button.bind(on_press=self.open_tasks)
        content.add_widget(task_button)

        add_button = RoundedButton("+ ADD NEW TASK", SUCCESS, size_hint_y=None, height=52)
        add_button.bind(on_press=self.open_add_task)
        content.add_widget(add_button)

        settings_button = RoundedButton("SETTINGS / PROFILE", SECONDARY, size_hint_y=None, height=52)
        settings_button.bind(on_press=self.open_settings)
        content.add_widget(settings_button)

        logout_button = RoundedButton("LOGOUT", DANGER, size_hint_y=None, height=52)
    ...
    def refresh_dashboard(self):
        ...
        user_tasks = [t for t in tasks if t.get("username") == app.current_user]
        completed = len([t for t in user_tasks if t.get("completed")])

        if app.current_user in users:
            self.welcome.text = f"Hello, {users[app.current_user]['full_name']}"
        else:
            self.welcome.text = "Hello"

        self.task_summary.text = f"{len(user_tasks)} saved tasks | {completed} completed"
...
class TaskManagementScreen(ProtectedScreen):
    def __init__(self, **kwargs):
        ...
        header = BoxLayout(orientation="horizontal", spacing=6, size_hint_y=None, height=46)

        back = RoundedButton("‹", SECONDARY, size_hint_x=None, width=48)
        back.bind(on_press=self.go_back)
        header.add_widget(back)

        header.add_widget(Label(text="Tasks", font_size=24, bold=True, color=PRIMARY))

        refresh = RoundedButton("↻", PRIMARY, size_hint_x=None, width=48)
        refresh.bind(on_press=self.refresh_tasks)
        header.add_widget(refresh)

        content.add_widget(header)

        add_button = RoundedButton("+ ADD NEW TASK", SUCCESS, size_hint_y=None, height=48)
        add_button.bind(on_press=self.open_add_task)
        content.add_widget(add_button)

        self.search_input = app_input("Search saved tasks", height=44)
        self.search_input.bind(text=self.search_tasks)
        content.add_widget(self.search_input)

        self.feedback = Label(text="", color=SUCCESS, font_size=13, size_hint_y=None, height=24)
    ...
    def load_user_tasks(self):
        ...
        self.tasks = [t for t in all_tasks if t.get("username") == app.current_user]

    def save_user_tasks(self):
        app = App.get_running_app()
        all_tasks = load_json(TASKS_FILE, [])
        others = [t for t in all_tasks if t.get("username") != app.current_user]
    ...
    def display_tasks(self, filtered_tasks=None):
        ...
        if not task_list:
            empty = RoundedBox(orientation="vertical", bg_color=CARD, radius=18, size_hint_y=None, height=130)
            empty.add_widget(Label(text="No saved tasks yet", font_size=18, bold=True, color=TEXT))
            empty.add_widget(Label(text="Tap '+ ADD NEW TASK' to create one.", font_size=13, color=MUTED))
        ...
        for task in task_list:
            ...
            card = RoundedBox(orientation="horizontal", bg_color=CARD, radius=20, size_hint_y=None, height=150)

            left = BoxLayout(orientation="vertical", spacing=2, size_hint_x=0.68)
            ...
            left.add_widget(Label(text=f"{status}", font_size=13, color=SUCCESS if task["completed"] else WARNING))
            left.add_widget(Label(text=f"{task['module']} • {task['priority']}", font_size=13, color=MUTED))
            left.add_widget(Label(text=f"Due: {task['due_date']}", font_size=13, color=MUTED))
            left.add_widget(Label(text=f"{task['notes']}", font_size=12, color=(0.25, 0.25, 0.25, 1)))

            right = BoxLayout(orientation="vertical", spacing=5, size_hint_x=0.32)

            complete = RoundedButton("DONE" if not task["completed"] else "UNDO", SUCCESS, size_hint_y=None, height=38)
            complete.bind(on_press=lambda instance, i=index: self.toggle_complete(i))
            right.add_widget(complete)

            edit = RoundedButton("EDIT", WARNING, size_hint_y=None, height=38)
            edit.bind(on_press=lambda instance, i=index: self.open_edit_task(i))
            right.add_widget(edit)

            delete = RoundedButton("DELETE", DANGER, size_hint_y=None, height=38)
            delete.bind(on_press=lambda instance, i=index: self.delete_task(i))
    ...
    def toggle_complete(self, index):
        ...
        self.feedback.text = "Task status refreshed"
        self.display_tasks()
    ...
    def delete_task(self, index):
        ...
        self.save_user_tasks()
        self.feedback.text = "Task deleted"
    ...
    def refresh_tasks(self, instance):
        self.load_user_tasks()
        self.display_tasks()
        self.feedback.text = "Tasks refreshed"
...
class AddTaskScreen(ProtectedScreen):
    def __init__(self, **kwargs):
        ...
        self.page_title = Label(text="Add Task", font_size=26, bold=True, color=PRIMARY, size_hint_y=None, height=58)
        ...
        self.due_date_input = app_input("Due date e.g. 2026-06-11")
        ...
        self.notes_input = app_input("Notes", multiline=True, height=90)
        card.add_widget(self.notes_input)

        self.message = Label(text="", color=DANGER, font_size=13, size_hint_y=None, height=32)
        card.add_widget(self.message)
        ...
        back = RoundedButton("BACK TO TASKS", PRIMARY, size_hint_y=None, height=48)
        back.bind(on_press=self.go_back)
    ...
    def save_task(self, instance):
        ...
        all_tasks = load_json(TASKS_FILE, [])
        user_tasks = [t for t in all_tasks if t.get("username") == app.current_user]
        other_tasks = [t for t in all_tasks if t.get("username") != app.current_user]
...
class SettingsScreen(ProtectedScreen):
    def __init__(self, **kwargs):
        ...
        content.add_widget(Label(text="Profile", font_size=26, bold=True, color=PRIMARY, size_hint_y=None, height=58))

        card = RoundedBox(orientation="vertical", bg_color=CARD, radius=22)
        self.profile_info = Label(text="", font_size=15, color=TEXT)
        card.add_widget(self.profile_info)
        content.add_widget(card)

        back = RoundedButton("BACK TO DASHBOARD", PRIMARY, size_hint_y=None, height=50)
        back.bind(on_press=self.go_back)
        content.add_widget(back)

        logout = RoundedButton("LOGOUT", DANGER, size_hint_y=None, height=50)
